...
- **Every custom dimension gets a description, written for a GA4-only reader.** GA4 shows no context about where a dimension comes from, so the description must stand alone: name the app + feature (e.g. "NextSteps admin template gallery"), where in the UI it happens, and the possible values when the set is small. No internal event names or repo jargon; max 150 chars. Example: "Where in the NextSteps admin template gallery a collection's public link was copied: the edit dialog or the publish success dialog."
