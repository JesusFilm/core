...
- **One source of truth eliminates _structural_ drift — but not _derived-logic_ drift.** Sharing `PublicGalleryPageData` and the component tree makes the NES-1682 regression (a section surviving in the preview after removal from the live page) structurally impossible. It does **not** protect logic each variant computes independently: the NES-1694 `FEATURED_COUNT` split drifted between `JourneyView` and `AdminView` precisely because each recomputed it. Shared _data_ is not shared _derivation_ — see the caveat under guidance rule 2.
