...
- R6. A minimal `apps/arclight/setupTests.ts` exists with `@testing-library/jest-dom/vitest`, the `stream/web` polyfill, and `configure({ asyncUtilTimeout: 2500 })` from `@testing-library/react` — so the very next spec file dropped in works without additional setup. **Deviation from journeys, corrected during PR review:** unlike `apps/journeys/setupTests.ts`, arclight does **not** install a `vi.mock('next/router', ...)` — arclight is App Router-only (`src/app/`, no `src/pages/`), so future tests should mock `next/navigation` (e.g. `useRouter`/`usePathname`/`useSearchParams`) instead. The setup file documents this inline so the next test author doesn't assume the mock was forgotten.
...
- **Reference migration #2 (sibling):** `apps/journeys/vitest.config.mts`, `apps/journeys/setupTests.ts`, `apps/journeys/tsconfig.json`, `apps/journeys/tsconfig.spec.json`, `apps/journeys/eslint.config.mjs` — read these in full and mirror the structure exactly, swapping `journeys` → `arclight` in path strings. Two journeys-specific items must be **dropped**: the `styled-jsx` alias (arclight has no styled-jsx usage) and the `vi.mock('next/router', ...)` block (arclight is App Router-only — see R6). Preserve the polyfills and the testing-library `configure(...)` call.
...
- **Add `setupTests.ts` even though there are no tests yet.** R6 is forward-looking: the very next test will need `@testing-library/jest-dom/vitest` matchers, the `stream/web` polyfill, and `configure({ asyncUtilTimeout: 2500 })`. Materializing the file now eliminates a "second migration" later. **Do not include the `vi.mock('next/router', ...)` block from journeys** — arclight is App Router-only (`src/app/`, no `src/pages/`); future tests should mock `next/navigation` instead. The original revision of this plan included the journeys mock as cargo-culted forward compat; PR-review feedback corrected the deviation before merge.
...
- `setupTests.ts`: include `@testing-library/jest-dom/vitest`, the `stream/web` polyfill block, and `configure({ asyncUtilTimeout: 2500 })`. **Do not** copy the `vi.mock('next/router', ...)` block from journeys — arclight is App Router-only, so leave it out and add an inline comment pointing future test authors at the `next/navigation` mocking pattern (see Key Technical Decisions). This was corrected during PR review after the initial commit cargo-culted the journeys mock.
