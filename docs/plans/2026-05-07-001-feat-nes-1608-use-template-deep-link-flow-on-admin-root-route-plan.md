...
- **No shared-hook extraction.** `UseTemplateDeepLink.tsx` and `libs/journeys/ui/.../CreateJourneyButton.tsx` carry ~45 lines of similar-shape code (the `journeyDuplicate` mutation call, the `useJourneyAiTranslateSubscription` wiring, the success/error snackbars). NES-1680 was filed and then cancelled — on re-examination most of the "duplication" is GraphQL-contract-mandated boilerplate (mutation variables, subscription handler shape) rather than logic that benefits from a shared abstraction, and the snackbar copy match is a coincidence of UX choice, not a shared contract. The two components remain legitimately independent implementations. If a third "use template" entry point ever shows up, revisit.
