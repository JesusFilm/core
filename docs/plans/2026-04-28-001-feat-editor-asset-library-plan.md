...
- **Update 2026-06-16 (NES-1634): the AI tab is personal-only, not team-merged.** The "Single merged grid per tab" decision above is amended: team-shared visibility ships only on the **Custom** tab's "Uploads" grid. The **AI** tab's "Generations" grid stays personal-only (`userId = caller`). Reason: AI images are created via `createImageBySegmindPrompt`, and the frontend never threads `journeyId` into that mutation, so AI rows are never `teamId`-tagged — merging on that grid would be a no-op. The `teamId` arg was removed from the AI query to avoid implying behavior that doesn't exist. To enable team-shared AI generations later, thread `journeyId` into the AI mutation (the resolver already derives `teamId` from it) and re-add the `teamId` prop to the AI grid.

#### Schema change

Prisma model update (`prisma-media`):
...
Each mutation that creates a `CloudflareImage` or `MuxVideo` row gains a required `journeyId` arg (the editor always knows which journey the user is in). Server-side flow per upload:

1. Look up `journey.teamId` from the journey ID (one Prisma read against `prisma-journeys`).
2. Verify caller has access to that journey via the existing journey-edit auth path (no new auth code).
...
- The Mux upload mutation (creates `MuxVideo`)
