...
Not covered locally: **`codegen`** is the one autofix.ci step that can still commit to your PR, and it currently **cannot be run locally at all**. The targets shell out to `npx apollo client:codegen`, but `apollo` is not a declared dependency — autofix.ci installs it globally at runtime (`npm i -g apollo graphql`), and in-repo `npx` fails outright with `EOVERRIDE` from the `next` override in `package.json`. So after changing a GraphQL schema or query, expect autofix.ci to commit the regenerated `__generated__` files; there is no local command that reproduces it today. The remaining steps cannot produce a commit at all: `type-check` and `subgraph-check` only report (and `subgraph-check` needs a Hive token), and `prisma-generate` writes nothing that is tracked.
