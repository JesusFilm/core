...
It is deliberately not gated, because it only works where `apollo` is already installed globally. The devcontainer installs it in [`post-create-command.sh`](.devcontainer/post-create-command.sh) (`npm i -g nx apollo graphql`) and autofix.ci does the same, which is why it works in both. On a plain host checkout `apollo` is absent unless you install it the same way (`npm i -g apollo graphql`; `pnpm setup:local` warns when it is missing), so `npx` tries to fetch it and fails with `EOVERRIDE` from the `next` override in `package.json`. It cannot be a devDependency: it is the deprecated Apollo CLI and peers on graphql 14/15 while the repo is on 16. Gating it would mean requiring that global install everywhere, which we chose not to do — install it globally, or expect autofix.ci to commit the regenerated files.
