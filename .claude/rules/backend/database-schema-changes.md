...
- **api-journeys (SDL-first):** Edit the relevant `.graphql` file in `apis/api-journeys/src/app/modules/<module>/` (e.g., `apis/api-journeys/src/app/modules/journeyProfile/journeyProfile.graphql`, NOT `schema.graphql`). Also update the resolver and its spec file if needed.
...
If `nx run-many -t codegen` logs "read the output from the cache" and the generated types (`apis/api-journeys/src/__generated__/graphql.ts`, `libs/shared/gql/src/__generated__/graphql-env.d.ts`) are missing a field that **is** present in `apis/api-gateway/schema.graphql`, the codegen cache key didn't capture the upstream gateway-schema change. Re-run with the cache disabled:
