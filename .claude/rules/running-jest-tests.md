...
For workspaces that have migrated to Vitest (have a `vitest.config.mts` instead — `apps/arclight`, `apps/journeys`, `apps/journeys-admin`, `apps/watch`, `apis/api-analytics`, `apis/api-media`), see `running-vitest-tests.md`.
