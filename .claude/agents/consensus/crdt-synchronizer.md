...
class LWWRegister {
  ...
  merge(otherRegister) {
    if (otherRegister.timestamp > this.timestamp || (otherRegister.timestamp === this.timestamp && otherRegister.nodeId > this.nodeId)) {
      const oldValue = this.value
...
class RGA {
  ...
  toArray() {
    return this.sequence.filter((vertex) => !this.tombstones.has(vertex.id)).map((vertex) => vertex.element)
  }
...
class DeltaStateCRDT {
  ...
  getDeltasSince(peerNode) {
    ...
    return this.deltaBuffer.filter((deltaEntry) => deltaEntry.vectorClock.isAfter(lastSync))
  }
  ...
  garbageCollectDeltas() {
    ...
    this.deltaBuffer = this.deltaBuffer.filter((deltaEntry) => deltaEntry.timestamp > cutoffTime)
  }
}
```
...
await this.mcpTools.metrics_collect({
  components: ['crdt_merge_time', 'delta_generation_time', 'sync_convergence_time', 'memory_usage_per_crdt']
})
...
class CRDTComposer {
  ...
  defineComposite(name, schema) {
    this.compositeTypes.set(name, {
      schema: schema,
      factory: (nodeId, replicationGroup) => this.createComposite(schema, nodeId, replicationGroup)
    })
  }
  ...
  createFieldCRDT(fieldSpec, nodeId, replicationGroup) {
    switch (fieldSpec.type) {
      case 'counter':
        return fieldSpec.decrements ? new PNCounter(nodeId, replicationGroup) : new GCounter(nodeId, replicationGroup)
      case 'set':
