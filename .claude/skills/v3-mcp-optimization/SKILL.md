...
export class ConnectionPool {
  ...
  private findAvailableConnection(endpoint: string): PooledConnection | null {
    for (const conn of this.pool.values()) {
      if (conn.isHealthy && conn.connection.endpoint === endpoint && Date.now() - conn.lastUsed < this.config.idleTimeoutMs) {
        return conn
      }
    }
    return null
...
export class MCPLoadBalancer {
  ...
  private leastConnectionsSelection(servers: ServerInstance[]): ServerInstance {
    return servers.reduce((least, current) => (current.currentConnections < least.currentConnections ? current : least))
  }

  private responseTimeSelection(servers: ServerInstance[]): ServerInstance {
    return servers.reduce((fastest, current) => (current.responseTime < fastest.responseTime ? current : fastest))
  }
...
```typescript
const mcpDashboard = {
  metrics: ['Request latency (p50, p95, p99)', 'Error rate by tool category', 'Connection pool utilization', 'Tool lookup performance', 'Memory usage trends', 'Cache hit rates (L1, L2, L3)'],
