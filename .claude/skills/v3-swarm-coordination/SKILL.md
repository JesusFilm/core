...
const phase4 = async () => {
  ...
  await Promise.all([
    ...
    ...agents.map((agent) => Task('Final polish', `Agent ${agent.id} final optimization`, agent.name))
  ])
}
```
