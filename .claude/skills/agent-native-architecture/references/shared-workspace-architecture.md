...
```typescript
const fileTools = [
  tool('read_file', 'Read a file from the workspace', { path: z.string().describe('File path') }, async ({ path }) => {
  ...
  tool('list_files', 'List files in a directory', { path: z.string().describe('Directory path') }, async ({ path }) => {
