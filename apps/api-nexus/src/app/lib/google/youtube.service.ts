...
export class GoogleYoutubeService {
  ...
  constructor(
    ...
    private readonly youtubeService: youtube_v3.Youtube
  ) {
    this.rootUrl = "https://www.googleapis.com/youtube/v3/channels";
    this.youtubeService = google.youtube("v3");
  }
  ...
  async uploadVideo(
    ...
    try {
      return await this.youtubeService.videos.insert(
        {
        ...
        },
        {
          onUploadProgress: (evt) => {
            ...
            void Promise.all([
              this.executeCallback(progressCallback, progress),
            ]);
          },
        }
      );
  ...
  async updateVideo(youtubeData: {
    ...
    try {
      ...
      uploadedYoutubeResponse = await this.youtubeService.videos.update({
        auth: this.googleOAuthService.authorize(
          youtubeData.token,
  ...
  async updateVideoThumbnail(youtubeData: {
    ...
    return await this.youtubeService.thumbnails.set({
      auth: this.googleOAuthService.authorize(
        youtubeData.token,
        "https://www.googleapis.com/auth/youtube https://www.googleapis.com/auth/youtubepartner https://www.googleapis.com/auth/youtube.force-ssl https://www.googleapis.com/auth/youtube.upload"
      ),
  ...
  async uploadCaption(youtubeData: {
    ...
    return await this.youtubeService.captions.insert(captionData);
  }
}
