...
| 4   | **Mid-stream client disconnect** ⭐ | From a terminal: `curl -N --max-time 0.5 -X POST https://journeys-9094-jesusfilm.vercel.app/api/chat -H 'content-type: application/json' -d '{"messages":[{"id":"1","role":"user","parts":[{"type":"text","text":"tell me a long story about the early church"}]}],"sessionId":"<uuid>","journeyId":"<id>"}'` | Generation `level: ERROR`, `statusMessage: 'pipe error'` (or similar). **This is the case the fix targets.** |
...
- **Payload shape:** `messages` must be `UIMessage` shape (with `id`
  and `parts: [{type:"text",text:"..."}]`). Sending `{role,content}`
  returns 500 because `convertToModelMessages` rejects it.
