...
pnpm setup:local              # or: pnpm setup:local --analytics  (adds clickhouse + plausible)
```

`setup:local` is idempotent. It checks the prerequisites, starts the services (`docker compose ... up --wait`),
creates the `test-user` Postgres role, runs `pnpm install`, and seeds `plausible_db` when the analytics profile
is on. Then continue with [environment variables](./02-environment-variables.mdx) and
...
| Task                         | Command                                                  |
| ---------------------------- | -------------------------------------------------------- |
| Start / stop services        | `pnpm services:up` (`services:up:analytics`), `pnpm services:stop` |
