...
export class TeamsPage {
  ...
  async clickThreeDotOptions(options) {
      await this.page.locator('li[data-testid="JourneysAdminMenuItem"] span', { hasText: options }).click()
  }
  async enterTeamName() {
  ...
  async clickCreateBtn() {
      await this.page.locator('div[data-testid="dialog-action"] button', { hasText: 'Create' }).click()
  }
  async verifyTeamCreatedSnackbarMsg() {
      await expect(this.page.locator('div#notistack-snackbar', { hasText: this.teamName + " created." })).toBeVisible()
  ...
  async selectLastTeam() {
      await this.page.locator('ul[role="listbox"] li[role="option"]',{hasNotText : 'Shared With Me'}).last().click()
  }
  async selectCreatedNewTeam() {
      await this.page.locator('ul[role="listbox"] li[role="option"]', { hasText: this.teamName }).click()
  }
  async verifyTeamNameUpdatedInTeamSelectDropdown() {
      await expect(this.page.locator('div[aria-haspopup="listbox"]')).toHaveText(this.teamName)
  }
  ...
  async verifyTeamRenameSnackbarMsg() {
      await expect(this.page.locator('div#notistack-snackbar')).toContainText(this.renameTeamName + " updated.")
      await expect(this.page.locator('div#notistack-snackbar')).toHaveCount(0, { timeout: 30000 })
  }
  async verifyRenamedTeamNameUpdatedInTeamSelectDropdown() {
      await expect(this.page.locator('div[aria-haspopup="listbox"]')).toHaveText(this.renameTeamName, { timeout: 60000 })
  }
  ...
  async verifyMemberAddedInMemberList() {
      await expect(this.page.locator('div[data-testid*="UserTeamInviteListItem"] p', { hasText: this.memberEmail })).toBeVisible()
  }
  async clickMemberPlusIcon() {
      await this.page.locator('div[data-testid="member-dialog-open-avatar"]').click()
  }
}
