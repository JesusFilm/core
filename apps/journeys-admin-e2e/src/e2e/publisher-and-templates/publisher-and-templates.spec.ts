...
import { Publisher } from '../../pages/publisher-page'
...
test.describe('Publisher page functionality', () => {
    ...
    test('Verify the user able to move the single template from Active, archived, Trash page', async ({ page }) => {
    ...
    test('Verify the user able to move the all template from Active, archived, Trash page', async ({ page }) => {
        const publisherPage = new Publisher(page)
        await publisherPage.navigateToPublisherPage()
    ...
    test('Verify the user able to display the publisher help window', async ({ page }) => {
        const publisherPage = new Publisher(page)
    ...
    //
    test('Publisher-> Select existing template -> Three dots on top right -> Template Settings -> Metadata', async ({ page }) => {
        const publisherPage = new Publisher(page)
        await publisherPage.navigateToPublisherPage() // navigating to the publisher page
        ...
        await publisherPage.clickTheDotOptionsInEditTemplatePage('Template Settings') // clicking Template Settings option from the three dot options
    ...
    test('Publisher-> Select existing template -> Three dots on top right -> Template Settings -> Categories', async ({ page }) => {
        const publisherPage = new Publisher(page)
        const journeyPage = new JourneyPage(page)
        ...
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Topics') // added filter on the Topics filter field for the template
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Felt Needs') // added filter on the 'Felt Needs' filter field for the template
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Holidays') // added filter on the Holidays filter field for the template
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Audience') // added filter on the Audience filter field for the template
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Genre') // added filter on the Genre filter field for the template
        await publisherPage.setFilterBelowCategoriesTabInTemplateSettingPopup('Collections') // added filter on the Collections filter field for the template
    ...
    test('Publisher-> Select existing template -> Three dots on top right -> Template Settings -> About', async ({ page }) => {
        const publisherPage = new Publisher(page)
    ...
    test('Publisher-> Select existing template -> Three dots on top right -> Language', async ({ page }) => {
        const publisherPage = new Publisher(page)
        const journeyLevelActions = new JourneyLevelActions(page)
        await publisherPage.navigateToPublisherPage() // navigating to the publisher page
        await publisherPage.getExistingTemplateName() // getting name of existing template
...
test.describe('Verify template page functionality', () => {
    ...
    test('create a new journey via use this template button', async ({ page }) => {
        const templatesPage = new TemplatePage(page)
        const journeyPage = new JourneyPage(page)
        const cardLevelActionPage = new CardLevelActionPage(page)
        await templatesPage.navigateToTempalatePage() //navigating to templates page
    ...
    test('preview a template from the journey template page', async ({ page, context }) => {
        const templatesPage = new TemplatePage(page)
    ...
    test('verify Help window displays via button on top right of the page', async ({ page }) => {
        const journeyLevelActions = new JourneyLevelActions(page)
        const templatesPage = new TemplatePage(page)
        await templatesPage.navigateToTempalatePage() // navigating to templates page
    ...
    test('Filter: Topics, holidays, felt needs, collections', async ({ page }) => {
        const templatesPage = new TemplatePage(page)
        await templatesPage.navigateToTempalatePage() // navigating to templates page
        await templatesPage.verifyFilterOfTopicsAndHolidaysAndFeltNeedsAndCollections('Topics') // Verifying that Topics templates are fetched by filtering the Topics.
        await templatesPage.verifyFilterOfTopicsAndHolidaysAndFeltNeedsAndCollections('Holidays') // Verifying that holiday templates are fetched by filtering the holidays.
        await templatesPage.verifyFilterOfTopicsAndHolidaysAndFeltNeedsAndCollections('Felt Needs') // Verifying that 'Felt Needs' templates are fetched by filtering the 'Felt Needs'.
        await templatesPage.verifyFilterOfTopicsAndHolidaysAndFeltNeedsAndCollections('Collections') // Verifying that Collections templates are fetched by filtering the Collections.
    })
