 ...
 */
test('admin can log in and see the Discover page', async ({ browser }) => {
  ...
  try {
    ...
    await expect(page.getByTestId('NavigationListItemProfile')).toBeVisible({
 ...
 */
test('new user who leaves before accepting terms returns to Terms and Conditions after signing in again', async ({
  browser
}) => {
 ...
 */
test('new user can register and see the Discover page in team mode', async ({
  browser
}) => {
