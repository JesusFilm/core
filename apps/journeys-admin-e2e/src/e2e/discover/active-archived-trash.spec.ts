...
test.describe("Verify user able to Active, Archived, Trash the journeys", () => {
  //Issue 2 : In Terms and Conditions page,The 'Next' button is not working properly 
  //Issue 3 : The error toast message is displaying after registration new account
  ...
  test('Verify the user able to move the single journeys from Active, archived, Trash page', async ({ page }) => {
    const journeyPage = new JourneyPage(page)
  ...
  test('Verify the user able to move the all journeys from Active, archived, Trash page', async ({ page }) => {
    const journeyPage = new JourneyPage(page)
    await journeyPage.clickCreateCustomJourney()// clicking on the create custom journey button
    await journeyPage.createAndVerifyCustomJourney() // creating the custom journey and verifing the created journey is updated in the active tab list
