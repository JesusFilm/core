...
test('media component returns 404 for non-existent ID', async ({ request }) => {
  const response = await request.get(
...
test('media component returns 404 for invalid language with no fallback content', async ({
  request
}) => {
  const response = await request.get(
