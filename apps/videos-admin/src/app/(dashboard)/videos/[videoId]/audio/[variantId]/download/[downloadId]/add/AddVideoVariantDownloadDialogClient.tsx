...
type AddVideoVariantDownloadDialogClientProps = {
  videoId: string
  variantId: string
  languageId: string
}
...
export function AddVideoVariantDownloadDialogClient({
  videoId,
  variantId,
  languageId
}: AddVideoVariantDownloadDialogClientProps): ReactElement {
  ...
  const isGenerateOption = (quality: string): boolean => quality === 'auto'
  const getButtonText = (quality: string): string =>
    isGenerateOption(quality) ? 'Generate' : 'Add'
  ...
  const handleSubmit = async (values: FormikValues): Promise<void> => {
      ...
      if (
        ...
        try {
          await enableMuxDownload({
            variables: { id: data.videoVariant.muxVideo.id, resolution: '720p' }
          })
          await enableMuxDownload({
            variables: { id: data.videoVariant.muxVideo.id, resolution: '360p' }
          })
          await enableMuxDownload({
            variables: { id: data.videoVariant.muxVideo.id, resolution: '270p' }
          })
          ...
          enqueueSnackbar(
            error instanceof Error
              ? error.message
              : 'Failed to create downloads from Mux',
            { variant: 'error' }
          )
        } finally {
          setIsLoading(false)
    ...
    if (uploadedFile == null || videoDimensions == null) {
          ...
          <Form>
            <Stack gap={2}>
              <FormControl
                fullWidth
                margin="normal"
                error={touched.quality === true && Boolean(errors.quality)}
              >
                ...
                <FormHelperText sx={{ minHeight: 20 }}>
                  {typeof errors.quality === 'string' ? errors.quality : null}
                </FormHelperText>
              </FormControl>
