...
export function Variants({ loading, variants }: VariantsProps): ReactElement {
  ...
  return (
    ...
    >
      {loading || items == null || items.length === 0 ? (
        <SectionFallback
          data-testid="VideoVariantsFallback"
          loading={loading}
          message={t('No variants')}
        />
      ) : (
              ...
              <Stack gap={1}>
                {items.map((variant, idx) => (
                  <ItemRow
                    ...
                    menu={{
                      actions: [
                        { label: 'View', handler: () => view(variant.id) }
                      ]
                    }}
                  />
                ))}
