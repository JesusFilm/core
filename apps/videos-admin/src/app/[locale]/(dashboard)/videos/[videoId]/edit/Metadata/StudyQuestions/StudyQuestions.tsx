  ...
  return (
    <Section
      ...
      action={{
        ...
        startIcon: <Plus2 />,
        onClick: () => openDrawer({ title: t('Create Study Question'), content: <h1>Create Study Question</h1>})
      }}
    >
      {totalQuestions > 0 ? (
        <OrderedList onOrderUpdate={updateOrder} items={studyQuestions}>
          {studyQuestions?.map(({ id, value }, idx) => (
            <OrderedItem
              ...
              actions={[
                { label: 'View', handler: (id) => openDrawer({ title: t('View Study Question'), content: <View studyQuestion={studyQuestions.find((sQ) => sQ.id === id )} />})}
              ]}
            />
          ))}
        </OrderedList>
