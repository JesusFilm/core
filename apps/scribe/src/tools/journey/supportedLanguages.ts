/**
 * Language ids accepted by the journeyAiTranslateCreate mutation. Kept in
 * sync with libs/journeys/ui/src/libs/useJourneyAiTranslateSubscription/supportedLanguages.ts —
 * scribe doesn't depend on the React lib, so this is duplicated rather than
 * imported.
 */
export const SUPPORTED_LANGUAGE_IDS: ReadonlyArray<string> = [
  // in i18n:
  '529', // English
  '4415', // Italiano, Italian
  '1106', // Deutsch, German, Standard
  '4451', // polski, Polish
  '496', // Français, French
  '20526', // Shqip, Albanian
  '584', // Português, Portuguese, Brazil
  '21028', // Español, Spanish, Latin American
  '20615', // 普通話, Chinese, Mandarin
  '3934', // Русский, Russian
  '22658', // Arabic Modern
  '7083', // Japanese
  '16639', // Bahasa Indonesia
  '3887', // Vietnamese
  '13169', // Thai
  '6464', // Hindi
  '12876', // Ukrainian
  '53441', // Arabic, Egyptian Modern Standard
  '1942', // Türkçe, Turkish
  '5541', // Serbian
  '6788', // Farsi, Western
  '3804', // Korean
  '1370', // Nepali
  // supported by AI model:
  '4791', // Amharic
  '139081', // Bengali
  '1964', // Bulgarian
  '21754', // Chinese (Simplified)
  '21753', // Chinese (Traditional)
  '1109', // Croatian
  '4432', // Czech
  '4454', // Danish
  '1269', // Dutch
  '4601', // Estonian
  '4820', // Finnish
  '483', // Greek
  '1341', // Hausa
  '6930', // Hebrew
  '1107', // Hungarian
  '371', // Kazakh
  '7519', // Latvian
  '7698', // Lithuanian
  '1927', // Malay
  '18259', // Mongolian, Halh
  '1254', // Myanmar (Burmese)
  '10393', // Norwegian
  '5546', // Romanian
  '13172', // Sinhala
  '5545', // Slovak
  '1112', // Slovenian
  '23178', // Swahili, Tanzania
  '4823', // Swedish
  '12551', // Tagalog
  '24309', // Tajik
  '5871', // Tamil
  '407', // Urdu
  '3888', // Uzbek, Northern
  '1308' // Yoruba
]
