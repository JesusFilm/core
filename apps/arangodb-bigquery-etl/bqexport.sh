...
arangoexport --type jsonl --compress-output true --overwrite true --server.authentication true --server.database $DATABASE_DB --server.username $DATABASE_USER --server.password $DATABASE_PASS --server.endpoint $DATABASE_URL --query "FOR v IN videos RETURN {_key: v._key, type: v.type, label: v.label, primaryLanguageId: v.primaryLanguageId, title: v.title, snippet: v.snippet, description: v.description, image: v.image, tagIds: v.tagIds, childIds: v.childIds, variants: v.variants, slug: v.slug, noIndex: v.noIndex, seoTitle: v.seoTitle, imageAlt: v.imageAlt}"
