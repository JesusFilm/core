...
vi.mock('next-i18next/pages', async (importOriginal) => ({
  ...(await importOriginal<typeof import('next-i18next/pages')>()),
...
describe('ContentMetadata', () => {
  ...
  it('closes download dialog when onClose is called', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('DialogDownload')).not.toBeInTheDocument()
    })
  })
