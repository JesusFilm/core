...
describe('AudioTrackSelect', () => {
  ...
  it('should call updateAudioLanguage when language selection changes', async () => {
    ...
    render(
      <MockedProvider mocks={[defaultGetLanguagesSlugMock]} addTypename={false}>
        <WatchProvider initialState={testInitialState}>
          <AudioTrackSelect />
        </WatchProvider>
      </MockedProvider>
    )
    ...
    // Verify mock is ready to capture calls to updateAudioLanguage
    expect(mockUpdateAudioLanguage).not.toHaveBeenCalled()
    expect(mockUpdateAudioLanguage).toBeDefined()
  })
})
