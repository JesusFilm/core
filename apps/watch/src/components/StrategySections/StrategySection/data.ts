export const newStrategyItems = [
  {
    post_id: 5899,
    post_type: 'mission-trip',
    post_type_label: 'Mission Trips',
    post_title: 'London Bridges 1 One Week',
    post_excerpt: '',
    post_date: 1671059126,
    post_date_formatted: '12/14/2022',
    post_modified: 1717597535,
    comment_count: 0,
    menu_order: 0,
    post_author: {
      user_id: 19,
      display_name: 'WillW',
      user_url: '',
      user_login: 'WillW'
    },
    images: {
      thumbnail: {
        url: 'https://develop.jesusfilm.org/wp-content/uploads/2022/11/1614904501165.jpg?w=150&h=150&crop=1',
        width: '150',
        height: '150'
      }
    },
    permalink:
      'https://develop.jesusfilm.org/mission-trips/london-bridges-1-one-week/',
    post_mime_type: '',
    taxonomies: {
      category: ['Uncategorized'],
      region: ['Europe'],
      strategy: [
        'Conversational Evangelism',
        'Distribution',
        'Family Mission Trips',
        'Mission Trip Training'
      ]
    },
    taxonomies_hierarchical: {
      category: {
        lvl0: ['Uncategorized']
      }
    },
    is_sticky: 0,
    content:
      'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the Middle East, escaping the blazing heat of their summer.\n\n\n\nIt’s an incredibly effective, low-key way to help influential people from the Middle East hear about Christ’s love. We will be involved in distribution of Arabic copies of the JESUS film, Magdalena: Released From Shame, and the New Testament while developing friendships. Out of curiosity, many will openly receive this gift from you in London, which they are hindered from receiving in their home country. Come share the love of Christ, warm conversation, and have a great time doing it.\n\n\n\nA unique highlight of this trip is our guided tour of biblical artifacts of the British Museum. It will be very educational and eye-opening and will enrich your faith as you hear the stories and see artifacts firsthand.\n\n\n\n\nThis is a Family Friendly trip for ages 6 and up.\n\n\n\n\nApplications are now closed.',
    record_index: 0,
    objectID: '5899-0',
    _snippetResult: {
      post_title: {
        value: 'London Bridges 1 One Week',
        matchLevel: 'none'
      },
      content: {
        value:
          'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the …',
        matchLevel: 'none'
      }
    },
    _highlightResult: {
      post_title: {
        value: 'London Bridges 1 One Week',
        matchLevel: 'none',
        matchedWords: []
      },
      taxonomies: {
        category: [
          {
            value: 'Uncategorized',
            matchLevel: 'none',
            matchedWords: []
          }
        ],
        region: [
          {
            value: 'Europe',
            matchLevel: 'none',
            matchedWords: []
          }
        ],
        strategy: [
          {
            value: 'Conversational Evangelism',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Distribution',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Family Mission Trips',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Mission Trip Training',
            matchLevel: 'none',
            matchedWords: []
          }
        ]
      },
      content: {
        value:
          'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the Middle East, escaping the blazing heat of their summer.\n\n\n\nIt’s an incredibly effective, low-key way to help influential people from the Middle East hear about Christ’s love. We will be involved in distribution of Arabic copies of the JESUS film, Magdalena: Released From Shame, and the New Testament while developing friendships. Out of curiosity, many will openly receive this gift from you in London, which they are hindered from receiving in their home country. Come share the love of Christ, warm conversation, and have a great time doing it.\n\n\n\nA unique highlight of this trip is our guided tour of biblical artifacts of the British Museum. It will be very educational and eye-opening and will enrich your faith as you hear the stories and see artifacts firsthand.\n\n\n\n\nThis is a Family Friendly trip for ages 6 and up.\n\n\n\n\nApplications are now closed.',
        matchLevel: 'none',
        matchedWords: []
      }
    },
    __position: 1
  },
  {
    post_id: 5900,
    post_type: 'mission-trip',
    post_type_label: 'Mission Trips',
    post_title: 'London Bridges 1 Two Week',
    post_excerpt: '',
    post_date: 1671058997,
    post_date_formatted: '12/14/2022',
    post_modified: 1717597565,
    comment_count: 0,
    menu_order: 0,
    post_author: {
      user_id: 19,
      display_name: 'WillW',
      user_url: '',
      user_login: 'WillW'
    },
    images: {
      thumbnail: {
        url: 'https://develop.jesusfilm.org/wp-content/uploads/2022/11/1614904501165.jpg?w=150&h=150&crop=1',
        width: '150',
        height: '150'
      }
    },
    permalink:
      'https://develop.jesusfilm.org/mission-trips/london-bridges-1-two-week/',
    post_mime_type: '',
    taxonomies: {
      category: ['Uncategorized'],
      region: ['Europe'],
      strategy: [
        'Conversational Evangelism',
        'Distribution',
        'Family Mission Trips',
        'Mission Trip Training'
      ]
    },
    taxonomies_hierarchical: {
      category: {
        lvl0: ['Uncategorized']
      }
    },
    is_sticky: 0,
    content:
      'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the Middle East, escaping the blazing heat of their summer.\n\n\n\nIt’s an incredibly effective, low-key way to help influential people from the Middle East hear about Christ’s love. We will be involved in distribution of Arabic copies of the JESUS film, Magdalena: Released From Shame, and the New Testament while developing friendships. Out of curiosity, many will openly receive this gift from you in London, which they are hindered from receiving in their home country. Come share the love of Christ, warm conversation, and have a great time doing it.\n\n\n\nA unique highlight of this trip is our guided tour of biblical artifacts of the British Museum. It will be very educational and eye-opening and will enrich your faith as you hear the stories and see artifacts firsthand.\n\n\n\n\nThis is a Family Friendly trip for ages 6 and up.\n\n\n\n\nApplications are now closed.',
    record_index: 0,
    objectID: '5900-0',
    _snippetResult: {
      post_title: {
        value: 'London Bridges 1 Two Week',
        matchLevel: 'none'
      },
      content: {
        value:
          'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the …',
        matchLevel: 'none'
      }
    },
    _highlightResult: {
      post_title: {
        value: 'London Bridges 1 Two Week',
        matchLevel: 'none',
        matchedWords: []
      },
      taxonomies: {
        category: [
          {
            value: 'Uncategorized',
            matchLevel: 'none',
            matchedWords: []
          }
        ],
        region: [
          {
            value: 'Europe',
            matchLevel: 'none',
            matchedWords: []
          }
        ],
        strategy: [
          {
            value: 'Conversational Evangelism',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Distribution',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Family Mission Trips',
            matchLevel: 'none',
            matchedWords: []
          },
          {
            value: 'Mission Trip Training',
            matchLevel: 'none',
            matchedWords: []
          }
        ]
      },
      content: {
        value:
          'Come with us to London to reach out to some of the hardest to reach people of the world. London is a main tourist destination for many visitors from the Middle East, escaping the blazing heat of their summer.\n\n\n\nIt’s an incredibly effective, low-key way to help influential people from the Middle East hear about Christ’s love. We will be involved in distribution of Arabic copies of the JESUS film, Magdalena: Released From Shame, and the New Testament while developing friendships. Out of curiosity, many will openly receive this gift from you in London, which they are hindered from receiving in their home country. Come share the love of Christ, warm conversation, and have a great time doing it.\n\n\n\nA unique highlight of this trip is our guided tour of biblical artifacts of the British Museum. It will be very educational and eye-opening and will enrich your faith as you hear the stories and see artifacts firsthand.\n\n\n\n\nThis is a Family Friendly trip for ages 6 and up.\n\n\n\n\nApplications are now closed.',
        matchLevel: 'none',
        matchedWords: []
      }
    },
    __position: 2
  }
]
