...
export function NewVideoContentPage(): ReactElement {
  ...
  const VideoContentWithCarousel = () => {
    const { loading, slides } = useVideoCarousel()
    const videos = slides.filter(slide => slide.source === 'video').map(slide => slide.video)

    // Handle video/insert selection from carousel
    const handleVideoSelectFromCarousel = useCallback((videoId: string) => {
    ...
    const handleSlideChangeFromCarousel = useCallback((activeIndex: number) => {
      setCurrentSlideIndex(activeIndex)
      if (slides[activeIndex]) {
        setCurrentPlayingId(slides[activeIndex].id)
      }
    }, [slides])

    return (
      <ContentPageBlurFilter>
        <NewVideoContentHeader loading={loading} videos={videos} />
        {((container?.childrenCount ?? 0) > 0 || childrenCount > 0) &&
          (slides.length > 0 || loading) && (
            <VideoCarousel
              ...
              loading={loading}
              onVideoSelect={handleVideoSelectFromCarousel}
              onSlideChange={handleSlideChangeFromCarousel}
            />
          )}
        <div
  ...
  const handleSlideChange = useCallback((activeIndex: number) => {
    setCurrentSlideIndex(activeIndex)
    // Note: We don't set currentPlayingId here as it should be handled by the carousel
  }, [])
  ...
  const VideoPageWithCarousel = () => {
    const { slides } = useVideoCarousel()
    ...
    const handleMuxInsertComplete = useCallback(() => {
      ...
      if (nextIndex < slides.length) {
        ...
        setCurrentPlayingId(slides[nextIndex].id)
      } else {
        setCurrentSlideIndex(0)
        setCurrentPlayingId(slides[0]?.id || id)
      }
    }, [currentSlideIndex, slides, id])
  ...
  return (
    <>
      ...
      <VideoCarouselProvider
        containerSlug={variantSlug}
        languageId={variant?.language.bcp47 ?? 'en'}
        autoProgress={false}
        enableMuxInserts={true}
        initialActiveVideoId={id}
      >
        <VideoPageWithCarousel />
      </VideoCarouselProvider>
    </>
  )
}
