...
describe('useAlgoliaRouter', () => {
  it('should extract languageId from URL when set as a menu facet', () => {
    const url = 'http://localhost:4300/watch?configure%5BruleContexts%5D%5B0%5D=home_page&menu%5BlanguageId%5D=529'
  ...
  it('should extract query from URL', () => {
    const url = 'http://localhost:4300/watch?query=love&configure%5BruleContexts%5D%5B0%5D=home_page'
    const {query, languageId, subtitleId} = extractQueryParams(url)
  ...
  it('should extract subtitleId from URL', () => {
    const url = 'http://localhost:4300/watch?configure%5BruleContexts%5D%5B0%5D=home_page&menu%5Bsubtitles%5D=529'
    const {query, languageId, subtitleId} = extractQueryParams(url)
    expect(query).toBeNull()
    expect(languageId).toBeNull()
