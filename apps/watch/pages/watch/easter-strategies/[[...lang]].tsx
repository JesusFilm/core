...
interface EasterStrategiesPageProps {
...
export default function EasterStrategiesPage({
  content
}: EasterStrategiesPageProps): ReactElement {
  return (
    <SnackbarProvider>
      <LanguageProvider>
...
export const getStaticProps: GetStaticProps<EasterStrategiesPageProps> = async (
  context
) => {
  const lang = context.params?.lang?.[0] ?? 'english'
  const contentId = 'easter-strategies'
