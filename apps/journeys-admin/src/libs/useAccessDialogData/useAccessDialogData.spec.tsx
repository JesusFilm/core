...
import { createMockInvite, createMockUserJourney, createMockUserTeam, mockGuestUser, mockOwnerUser, mockTeamMemberUser } from './useAccessDialogData.mocks'
...
describe('useAccessDialogData', () => {
  ...
  describe('when current user is journey owner', () => {
    ...
    it('should identify current user as owner and create synthetic team data', () => {
      const result = useAccessDialogData(mockData, undefined, mockOwnerUser.email)
      ...
      expect(result.enhancedTeamData?.userTeams[0].role).toBe(UserTeamRole.manager)
      expect(result.enhancedTeamData?.userTeams[0].user.email).toBe(mockOwnerUser.email)
  ...
  describe('when current user is guest', () => {
    ...
    it('should identify current user as guest and show owner in team section', () => {
      const result = useAccessDialogData(mockData, undefined, mockGuestUser.email)
  ...
  describe('when journey has existing team', () => {
    ...
    it('should add synthetic owner to existing team when owner is not team member', () => {
      ...
      expect(result.enhancedTeamData?.userTeams[1].user.email).toBe(mockTeamMemberUser.email)
    })
    ...
    it('should not duplicate owner when owner is already team member', () => {
      ...
      const result = useAccessDialogData(dataWithOwnerInTeam, undefined, mockGuestUser.email)
    ...
    const mockData: GetJourneyWithPermissions = {
      journey: {
        ...
        team: {
          __typename: 'Team', 
          id: 'team-id',
          userTeams: [createMockUserTeam(mockTeamMemberUser, UserTeamRole.member)]
        },
        userJourneys: [
          createMockUserJourney(mockOwnerUser, UserJourneyRole.owner),
          createMockUserJourney(mockGuestUser, UserJourneyRole.editor),
          createMockUserJourney({ ...mockGuestUser, id: 'request-user', email: 'request@example.com' }, UserJourneyRole.inviteRequested)
        ]
      }
    }

    it('should categorize users correctly', () => {
      const result = useAccessDialogData(mockData, undefined, 'other@example.com')
      ...
      expect(result.pendingRequests[0].role).toBe(UserJourneyRole.inviteRequested)
    })
  })
  ...
  describe('email collection', () => {
    ...
    it('should collect all emails from user journeys and pending invites', () => {
      const result = useAccessDialogData(mockData, mockUserInviteData, undefined)
  ...
  describe('current user identification', () => {
    ...
    it('should identify current user in both journey and team contexts', () => {
      const result = useAccessDialogData(mockData, undefined, mockTeamMemberUser.email)
