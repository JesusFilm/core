...
export function useAccessDialogData(
  data: GetJourneyWithPermissions | undefined,
  userInviteData: any,
  ...
  const currentUserJourney = findCurrentUserJourney(
    userJourneys,
    currentUserEmail
  )
  ...
  const enhancedTeamData = createEnhancedTeamData(
    data,
    userJourneys,
    teamMembers
  )
  ...
  // Categorize users
  const { guestUsers, pendingRequests } = categorizeUserJourneys(
    userJourneys,
    teamMembers
  )
...
function findCurrentUserJourney(
...
function findJourneyOwner(
  userJourneys: UserJourney[]
): UserJourney | undefined {
  return userJourneys.find((uj) => uj.role === UserJourneyRole.owner)
}

function isOwnerAlreadyInTeam(
  journeyOwner: UserJourney | undefined,
  teamMembers: UserTeam[]
): boolean {
...
function createEnhancedTeamData(
  data: GetJourneyWithPermissions | undefined,
  userJourneys: UserJourney[],
  teamMembers: UserTeam[]
): JourneyTeam | undefined {
...
function categorizeUserJourneys(
  userJourneys: UserJourney[],
  teamMembers: UserTeam[]
) {
