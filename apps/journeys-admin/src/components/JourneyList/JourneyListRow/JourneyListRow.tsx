...
export function JourneyListRows({
  ...
  if (!smUp) {
    return (
      ...
      >
        <List disablePadding>
          {rows.map(({ journey, variant }, index) => (
...
function JourneyListMobileRow({
  journey,
  variant = JourneyCardVariant.default,
  showDivider
}: JourneyListRowProps & { showDivider: boolean }): ReactElement {
  const { t } = useTranslation('apps-journeys-admin')
  const isNavigating = useNavigationState()
  const [isImageLoading, setIsImageLoading] = useState(true)
  const languageName = journey.language.name.find(
  ...
  return (
    <>
      <ListItem
        ...
        secondaryAction={<OwnerAvatar journey={journey} />}
      >
        <Thumbnail
          journey={journey}
          isNavigating={isNavigating}
...
function OwnerAvatar({ journey }: { journey: Journey }): ReactElement {
