...
const cardCtaResult = jest.fn(() => ({
  data: cardCtaCreate
}))

const cardCtaCreateMock: MockedResponse<CardCtaCreate, CardCtaCreateVariables> =
  {
    request: {
...
describe('CardCta', () => {
  ...
  it('updates loading state when clicked', async () => {
    ...
    fireEvent.click(getByRole('button', { name: 'Card CTA Template' }))
    await waitFor(() => expect(cardCtaResult).toHaveBeenCalled())
