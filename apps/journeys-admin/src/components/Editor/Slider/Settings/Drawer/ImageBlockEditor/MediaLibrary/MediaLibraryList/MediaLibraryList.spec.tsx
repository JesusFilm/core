...
describe('MediaLibraryList', () => {
  ...
  it('should hide multiple tiles when several images fail to load', () => {
    render(<MediaLibraryList images={images} handleSelect={jest.fn()} />)
    const imgA = screen
      .getByTestId('media-library-image-a')
      .querySelector('img') as HTMLImageElement
    const imgC = screen
      .getByTestId('media-library-image-c')
      .querySelector('img') as HTMLImageElement
    fireEvent.error(imgA)
    fireEvent.error(imgC)
    expect(
      screen.queryByTestId('media-library-image-a')
    ).not.toBeInTheDocument()
    expect(
      screen.queryByTestId('media-library-image-c')
    ).not.toBeInTheDocument()
    expect(screen.getByTestId('media-library-image-b')).toBeInTheDocument()
  })

  it('should keep a tile hidden across re-renders with the same images', () => {
    const { rerender } = render(
      <MediaLibraryList images={images} handleSelect={jest.fn()} />
    )
    const imgB = screen
      .getByTestId('media-library-image-b')
      .querySelector('img') as HTMLImageElement
    fireEvent.error(imgB)
    rerender(<MediaLibraryList images={images} handleSelect={jest.fn()} />)
    expect(
      screen.queryByTestId('media-library-image-b')
    ).not.toBeInTheDocument()
    expect(screen.getByTestId('media-library-image-a')).toBeInTheDocument()
  ...
  it('should hide the selected tile when its image fails to load', () => {
    render(
      <MediaLibraryList
        images={images}
        selectedSrc="https://example.com/b/public"
