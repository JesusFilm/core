...
describe('mediaErrorMessage', () => {
  it('maps known reasons to specific messages', () => {
    ...
    expect(mediaErrorMessage('YOUTUBE_UNAVAILABLE', t)).toMatch(
      /cannot be embedded/i
    )
    ...
    expect(mediaErrorMessage('EMBED_HOST_NOT_ALLOWED', t)).toMatch(
      /cannot embed/i
    )
  ...
  it('falls back to a generic message for an unknown reason', () => {
    expect(mediaErrorMessage('SOMETHING_NEW', t)).toMatch(
      /couldn't add that media/i
    )
  })
})
