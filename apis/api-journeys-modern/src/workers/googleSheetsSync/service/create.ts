...
import { GoogleSheetsSyncCreateJobData } from '../queue'
...
/**
 * Create service: writes initial data to a newly created sheet.
 * This runs inside the worker to prevent locking issues with concurrent operations.
 * Unlike backfill, this doesn't clear existing data (sheet is expected to be new/empty).
 */
export async function createService(
  job: Job<GoogleSheetsSyncCreateJobData>,
  logger?: Logger
): Promise<void> {
  const {
    journeyId,
  ...
  if (sync == null) {
    logger?.warn(
      ...
      'Sync not found or deleted, skipping initial export'
    )
    return
  }
  ...
  if (journey == null) {
    logger?.warn({ journeyId }, 'Journey not found, skipping initial export')
    return
  }
  ...
  if (blocksToUpdate.length > 0) {
    try {
      ...
      logger?.error(
        ...
        'Failed to update exportOrder on blocks after initial export'
      )
    }
  }
