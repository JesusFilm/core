-- Final SQL script to generate 100 visitors with unique users and events (test version)
...
visitor_data AS (
    SELECT gen_random_uuid() as "id",
        ...
        NOW() - (random() * interval '7 days') as "createdAt",
        CASE
            (random() * 3)::int
            WHEN 0 THEN 'US'
            WHEN 1 THEN 'CA'
...
event_data AS (
    ...
    FROM inserted_journey_visitors jv
        CROSS JOIN generate_series(1, (random() * 2 + 1)::int) -- 1-3 events per visitor
