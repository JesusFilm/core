...
// Input types for ImageBlock operations
const ImageBlockCreateInput = builder.inputType('ImageBlockCreateInput', {
  fields: (t) => ({
    id: t.id({
      required: false,
      description:
...
const ImageBlockUpdateInput = builder.inputType('ImageBlockUpdateInput', {
  fields: (t) => ({
    parentBlockId: t.id({ required: false }),
    src: t.string({ required: false }),
