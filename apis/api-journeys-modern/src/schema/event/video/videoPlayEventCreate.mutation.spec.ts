...
describe('videoPlayEventCreate', () => {
  ...
  const VIDEO_PLAY_EVENT_CREATE = graphql(`
  ...
  it('creates a VideoPlayEvent when authorized', async () => {
    ...
    const result = await authClient({
      document: VIDEO_PLAY_EVENT_CREATE,
      variables: {
        input: {
          id: 'eventId',
          blockId: 'blockId',
    ...
    expect(result).toEqual({
      data: {
        videoPlayEventCreate: expect.objectContaining({
          id: 'eventId',
          journeyId: 'journeyId'
        })
      }
