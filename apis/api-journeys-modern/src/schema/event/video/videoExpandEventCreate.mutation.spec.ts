...
describe('videoExpandEventCreate', () => {
  ...
  const VIDEO_EXPAND_EVENT_CREATE = graphql(`
  ...
  it('creates a VideoExpandEvent when authorized', async () => {
    const createdEvent = {
      id: 'eventId',
    ...
    const result = await authClient({
      document: VIDEO_EXPAND_EVENT_CREATE,
      variables: {
        input: {
          id: 'eventId',
          blockId: 'blockId',
    ...
    expect(result).toEqual({
      data: {
        videoExpandEventCreate: expect.objectContaining({
          id: 'eventId',
          journeyId: 'journeyId'
        })
      }
