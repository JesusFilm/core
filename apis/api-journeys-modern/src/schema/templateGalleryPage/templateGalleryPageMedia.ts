...
// PUBLIC projection — reachable on the unauthenticated `templateGalleryPageBySlug`
// path (NES-1547). Both payload slots may be populated on the row at once, but
// this type exposes ONLY the active payload (the one `type` selects). The
// inactive/parked payload is never emitted here, so a hidden upload or link
// can't leak to anonymous viewers.
//
// Deliberately NOT exposed:
//   - `muxVideoId` — the raw FK is internal-only (authenticated admin type only).
...
// ADMIN projection — authenticated reads only (`templateGalleryPage`,
// `templateGalleryPages`, and the page mutations). Exposes the FULL row: both
// retained payload slots AND the raw `muxVideoId`, so the editor can show a
// parked upload/link and let the user switch back to it without re-entering it.
// Safe because every read path returning this type is gated by team membership.
export const TemplateGalleryPageMediaAdminRef = builder.prismaObject(
  'TemplateGalleryPageMedia',
  {
    variant: 'TemplateGalleryPageMediaAdmin',
    description:
