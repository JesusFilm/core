...
export async function deleteJourneysData(
  ...
  try {
    // maxWait / timeout are generous to handle power users with large datasets.
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        for (const ut of userTeams) {
          const classification = classifyTeam(
            userId,
            ut.role,
            ut.team.userTeams
          )
      ...
      // timeout: total wall-clock budget for the entire transaction — generous
      // to handle power users who own many journeys with large event histories.
