...
## Fake Video Import Slack Test

The fake video import Slack test sends real issue-reporting Slack notifications
with fake video import failure payloads. It does not create database, R2, or Mux
records. Failure notifications include an Open in Nexus action when the fake
payload has a video ID.

### Usage

Load the api-media Slack environment variables first, then run:
...
- `SLACK_VIDEO_ADMIN_BOT_TOKEN`: Slack bot token used by api-media video notifications
- `SLACK_DATA_LANGS_CHANNEL_ID`: Slack channel ID to post the test messages into
- `SERVICE_ENV`: Optional. Use `prod` for production Nexus links; anything else uses stage links

### Options

```bash
pnpm exec nx run api-media:fake-video-import-slack-test -- --send --scenario=r2
pnpm exec nx run api-media:fake-video-import-slack-test -- --send --scenario=mux
pnpm exec nx run api-media:fake-video-import-slack-test -- --send --scenario=queue
pnpm exec nx run api-media:fake-video-import-slack-test -- --send --scenario=variant
pnpm exec nx run api-media:fake-video-import-slack-test -- --send --video-id=fake-import-123
```

Scenarios:

- `all`: Send fake R2, Mux, queue, and variant failure notifications
- `r2`: Send a fake R2 asset creation failure notification
- `mux`: Send a fake Mux asset creation failure notification
- `queue`: Send a fake upload queue failure notification
- `variant`: Send a fake video variant creation failure notification
