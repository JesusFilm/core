...
const EXCLUDED_TABLES = ['MuxVideo', 'CloudflareR2', 'UserMediaRole']
 ...
 * Excludes specified tables and only includes CloudflareImage rows with userId 'system'
 */
async function executePgDump(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Add table-specific filtering with WHERE clauses
    // Only export CloudflareImage rows with userId 'system'
    const whereClauseArgs = [
      '--table',
    ...
    logger.info(
      {
        ...
        cloudflareImageFilter: 'userId = system'
      },
      'Export filters'
    )
 ...
 * Excludes specified tables and filters CloudflareImage records
 */
