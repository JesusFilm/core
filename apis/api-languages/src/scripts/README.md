...
Imports language rows into `Language` and `LanguageName` (English label, `languageId=529`, and — when WESS supplies one — the language's own autonym, `languageId=<that language's own id>`).
...
- **`NATIVE_LAN_NAME`** (autonym): the language's native/autonym name, mapped onto `nativeName`. When present (non-blank after trim), the importer upserts a second `LanguageName` row shaped `{ parentLanguageId: <this language's id>, languageId: <this language's id>, primary: true }`. Immediately before that write it flips `primary` to `false` on **every other** name row for the language — the English label, and anything an older run or a manual edit left flagged primary in some third language — so the autonym ends up as the language's sole Primary Name. That demotion is unconditional: it does not depend on WESS having supplied `LAN_NAME` on the same run, because a run can carry an autonym and no English gloss while a stored English row is still primary. Blank/whitespace-only values are treated as absent — no different from any other field read via `readStringField`. Absence is a no-op, not a retraction: a language that already has a stored autonym keeps that row, still `primary: true`, even if a later run's WESS response temporarily omits `NATIVE_LAN_NAME` — and on that run the English row's `primary` flag is left untouched rather than promoted back to `true`, so the two rows never disagree about which one is primary. English's own WESS row carries `NATIVE_LAN_NAME: "English"`, so it flows through this same branch (`parentLanguageId: '529', languageId: '529', value: 'English'`) — no special case.
