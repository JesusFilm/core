...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  ...
  container_definitions = jsonencode([
    ...
    # mail container
    {
      name      = "${local.ecs_task_definition_family}-mail-app"
      image     = "bytemark/smtp:latest"
      essential = true
      cpu       = var.mail_service_config.cpu
      memory    = var.mail_service_config.memory
      portMappings = [
        {
          containerPort = var.mail_service_config.container_port
          hostPort      = var.mail_service_config.host_port
          protocol      = "tcp"
        }
      ]
      logConfiguration = {
        ...
        options = {
          ...
          TLS         = "on"
          compress    = "gzip"
          dd_service  = var.mail_service_config.name
          dd_source   = var.dd_source
          dd_tags     = "env:${var.env}"
          provider    = "ecs"
          retry_limit = "2"
