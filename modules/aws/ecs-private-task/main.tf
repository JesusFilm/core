resource "aws_ecr_repository" "ecr_repository" {
  ...
  name = lower("core-${var.service_config.name}")
}
...
resource "aws_ecs_service" "ecs_service" {
  ...
  load_balancer {
    target_group_arn = var.internal_alb_target_groups[var.service_config.name].arn
    container_name   = var.service_config.name.name
    container_port   = var.service_config.name.container_port
  }
}

resource "aws_appautoscaling_target" "service_autoscaling" {
  max_capacity       = var.service_config.name.auto_scaling.max_capacity
  min_capacity       = var.service_config.name.auto_scaling.min_capacity
...
resource "aws_appautoscaling_policy" "ecs_policy_memory" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    target_value = var.service_config.name.memory.target_value
  }
}
