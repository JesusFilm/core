...
- **OpenRouter sees content.** Both `--llm-scrub` and the always-on theme synthesis send regex-scrubbed user content to OpenRouter. Under `--translate`, assistant replies are also sent and are not regex-scrubbed. Verify OpenRouter's data-retention/no-training terms before the first production run — NES-1562 covers this.
...
- **Machine translation accuracy is not guaranteed.** `--translate` output is model-generated and may mistranslate, especially idiom, nuance, and religious terminology. The **original message is always retained and never altered** — treat it as the source of truth and the English as an aid only. Translation also sends content to OpenRouter, including assistant replies that are not regex-scrubbed (see "OpenRouter sees content" above).
