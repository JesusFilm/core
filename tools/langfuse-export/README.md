...
- **Per-region "fingerprint" section (NES-1577).** Each report now carries a `By region` block per IP-country (from the `ipCountry` trace metadata NES-1574 captures). Cards show real vs synthetic conversation counts, multi-turn share, the "long conversation" share (Aaron's HollowBible `>10 messages` threshold, mapped to `>5 turns`), per-region language mix, and the region's top questions (same eligibility as the global rollup: real-session, multi-turn). Themes are annotated with a per-country breakdown (`Geo: NZ ×5 · US ×2`). Per-country LLM theme synthesis is deliberately off for v1 — at current data volume it would hallucinate; layer it on once production volume meets a minimum per-country threshold.
